...
namespace=$1 #Kubernetes namespace where team master is deployed.
teamMasterStatefulsetName=$2 #The statefulset name of the team master deployment.
...
if [ -z $namespace ] || [ -z $teamMasterStatefulsetName ] || [ -z $s3Bucket ] || [ -z $backupFilePath ] || [ -z $rescueContainerImage ]
then
  echo "Execution parameters missing. Loading variables from config file."
  source config
fi

echo "Scale down Team Master pods to 0 replicas"
kubectl --namespace=$namespace scale statefulset/$teamMasterStatefulsetName --replicas=0

#Launch rescue pod attaching the pvc
persistentVolumeClaim=$(kubectl -n $namespace get statefulset $teamMasterStatefulsetName -o jsonpath="{.spec.volumeClaimTemplates[0].metadata.name}")-${teamMasterStatefulsetName}-0
...
kubectl --namespace=$namespace scale statefulset/$teamMasterStatefulsetName --replicas=1
